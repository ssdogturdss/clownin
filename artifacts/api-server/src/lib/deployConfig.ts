...
A \`.github/workflows/fly-deploy.yml\` and \`fly.toml\` are included in this repo.
Every push to \`main\` deploys automatically — no CLI needed.

**Setup (one time):**
1. Create a Fly.io account at [fly.io](https://fly.io) and create a new app in their dashboard
2. Open \`fly.toml\` in this repo and replace \`your-app-name\` with your actual Fly.io app name
3. Generate an API token: Fly.io dashboard → **Account** → **Access Tokens** → **Create token**
4. Add it to your GitHub repo: **Settings** → **Secrets and variables** → **Actions** → **New secret**
   - Name: \`FLY_API_TOKEN\`
   - Value: your token
5. Push to \`main\` — your app deploys automatically to \`<app>.fly.dev\`
...
function flyDeployWorkflow(): string {
...
function flyToml(port: number): string {
  return `# Generated by Clownin — replace "your-app-name" with your actual Fly.io app name.
# Create an app at https://fly.io/dashboard → New App, then copy the name here.
app = "your-app-name"
primary_region = "iad"
...
export function generateContainerFiles(files: FileEntry[]): {
  ...
  if (type === "node-server") {
    ...
    if (!map.has(".github/workflows/fly-deploy.yml")) {
      map.set(".github/workflows/fly-deploy.yml", flyDeployWorkflow());
    }
    if (!map.has("fly.toml")) {
      map.set("fly.toml", flyToml(3000));
    }
